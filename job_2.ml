...
let analyze : unit =
  ...
  then (
    let l = read_line() in
    print_endline l;
    (*let l : string ref = {contents = read_line()} in
    let ret : match_score list ref = {contents = []} in
    try (
    while true do
      let ss = String.split_on_char '\t' !l in
      print_endline !l;
      let rec read_data_line (ss : string list) : match_score =
        ...
        let score1 : int = int_of_string arr.(5) in
        let score2 : int = int_of_string arr.(6) in
        
        PL (team1, score1, team2, score2, 40) in
      ret.contents <- (read_data_line ss)::ret.contents;
      l.contents <- read_line()
    done;)
    with e -> ();*)
